    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex items-center justify-center">
            <div className="relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-sm font-medium text-slate-500 dark:text-slate-400">/ {formatTime(survey.isActive ? survey.requiredTime : configuration.baseStation.surveyDuration)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center p-4 rounded-xl bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800/80">
              <Clock className="size-5 mx-auto mb-2 text-blue-500" />
              <div className="text-2xl font-bold font-mono text-slate-900 dark:text-slate-50">{formatTime(survey.isActive ? survey.requiredTime : configuration.baseStation.surveyDuration)}</div>
      ...
      {/* <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 shadow-sm">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-slate-900 dark:text-slate-50">Position Information</CardTitle>
            ...
            <div className="flex gap-2">
              {isPositionFixed && (
                <Badge className="bg-emerald-500 text-white border-none gap-2">
                  <CheckCircle2 className="size-3" />
                  Fixed
                </Badge>
              )}
              <Badge variant="outline" className="gap-2 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300">
                <MapPin className="size-3" />
                ±{survey.position.accuracy > 0 ? survey.position.accuracy.toFixed(3) : 'NIL'}m
              </Badge>
            </div>
        ...
        <CardContent className="space-y-4">
          <Tabs value={coordinateFormat} onValueChange={(v) => setCoordinateFormat(v as any)}>
            <TabsList className="grid w-full grid-cols-2 bg-slate-100 dark:bg-slate-800/50">
              <TabsTrigger value="Global" className="data-[state=active]:bg-white dark:data-[state=active]:bg-slate-700 data-[state=active]:text-slate-900 dark:data-[state=active]:text-slate-50 text-slate-500 dark:text-slate-400">Global</TabsTrigger>
              <TabsTrigger value="Local" className="data-[state=active]:bg-white dark:data-[state=active]:bg-slate-700 data-[state=active]:text-slate-900 dark:data-[state=active]:text-slate-50 text-slate-500 dark:text-slate-400">Local</TabsTrigger>
            </TabsList>
          </Tabs>

          <div className="space-y-3 font-mono">
            <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800/50">
              <div className="text-xs font-sans font-semibold text-slate-500 dark:text-slate-400 mb-1">
                {coordinateFormat === 'Global' ? 'Latitude' : 'Mean X (Easting)'}
              </div>
              <div className="text-lg font-semibold text-slate-900 dark:text-slate-50">{coords.lat}</div>
            </div>
            ...
            <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800/50">
              ...
              <div className="text-lg font-semibold text-slate-900 dark:text-slate-50">{coords.lon}</div>
            </div>
            ...
            <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800/50">
              ...
              <div className="text-lg font-semibold text-slate-900 dark:text-slate-50">{coords.alt} <span className="text-sm">m</span></div>
            </div>
          </div>

          <Button variant="outline" className="w-full gap-2 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800" onClick={copyCoordinates}>
            <Copy className="size-4" />
            Copy {coordinateFormat} Coordinates
          </Button>
        </CardContent>
