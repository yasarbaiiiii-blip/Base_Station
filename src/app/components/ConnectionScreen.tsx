...
import { 
  Wifi, Bluetooth, RotateCw, Lock, Cpu, Zap, ShieldCheck, 
  ChevronRight, Eye, EyeOff, Orbit, Lightbulb, Radar, 
  SearchCode, Activity, Terminal, Waves
} from 'lucide-react';
...
export const ConnectionScreen: React.FC = () => {
  ...
  const getSmartSuggestion = () => {
    ...
    if (isScanning) return "Scanning radio spectrum...";
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#020617] transition-colors duration-700 flex flex-col items-center justify-start lg:justify-center p-4 md:p-6 lg:p-10 pt-safe overflow-hidden">
      ...
      <main className="w-full max-w-[1400px] grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        {/* ── COLUMN 1: Protocol Select (Desktop Sidebar) ── */}
        <div className="hidden lg:flex lg:col-span-3 space-y-4 flex-col justify-center animate-in slide-in-from-left-8 duration-700">
          ...
          {[
            ...
            { id: 'wifi', icon: Wifi, label: 'WLAN Link', desc: 'Handshake Mode' },
        ...
        {/* ── COLUMN 2: Interaction Stage (Center) ── */}
            ...
            <div className="p-6 md:p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 flex items-center justify-between">
              <div className="flex items-center gap-4">
                 <div className="h-10 w-10 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-center shadow-inner">
                    <Lightbulb className={`size-5 ${isConnecting ? 'text-blue-500 animate-pulse' : 'text-slate-400'}`} />
                 </div>
                 <div>
                    <p className="text-[9px] font-black uppercase tracking-[0.2em] text-blue-500">Intelligent Trace</p>
                    <p className="text-xs md:text-sm font-medium text-slate-600 dark:text-slate-300 leading-tight">{getSmartSuggestion()}</p>
                 </div>
              </div>
              {connectionMode !== 'auto' && (
                <button 
                  onClick={() => handleScan(connectionMode)} 
                  disabled={isScanning} 
                ...
                <div className="space-y-4 animate-in slide-in-from-right-8 duration-500">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {availableWiFiNetworks.map((n) => (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className={`p-2.5 rounded-xl ${selectedNetwork === n.ssid ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                            <Wifi size={18} />
                          </div>
                          <div>
                            <span className="text-sm font-semibold dark:text-slate-100 block">{n.ssid}</span>
                            <p className="text-[10px] font-normal text-slate-400 uppercase mt-0.5">{n.signalStrength} dBm • {n.secured ? 'WPA2' : 'Open'}</p>
                          </div>
                        </div>
                        <div className="flex gap-1 items-end h-4">
                           {Array.from({ length: 4 }).map((_, i) => (
                             <div key={i} className={`w-1.5 rounded-full transition-all duration-700 ${i < getSignalBars(n.signalStrength) ? (selectedNetwork === n.ssid ? 'bg-blue-500' : 'bg-emerald-500') : 'bg-slate-200 dark:bg-slate-800'}`} style={{ height: `${(i+1)*25}%` }} />
                           ))}
                        </div>
                      </div>
                    ))}
        ...
        {/* ── COLUMN 3: Live Telemetry Sidebar (Desktop Only) ── */}
