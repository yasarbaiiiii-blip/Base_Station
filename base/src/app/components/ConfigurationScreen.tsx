...
const SectionCard: React.FC<{
  ...
  if (isMobile) {
    return (
      <>
        ...
        <div onClick={() => setIsOpen(true)} className="flex items-center justify-between p-4 rounded-xl bg-card border border-border shadow-sm active:scale-95 transition-transform cursor-pointer">
        ...
        {isOpen && (
          <div className="fixed inset-0 z-50 flex flex-col bg-background animate-in slide-in-from-right-4 duration-200">
             
             {/* Sticky Native Header */}
             <div className="flex items-center gap-3 p-4 border-b border-border bg-background/90 backdrop-blur-md shadow-sm shrink-0 pt-safe">
                <Button variant="ghost" onClick={() => setIsOpen(false)} className="rounded-full h-9 w-9 p-0 text-muted-foreground hover:bg-muted/50">
             ...
             <div className="p-4 border-t border-border bg-background/90 backdrop-blur-md pb-safe shrink-0">
                <Button
                  onClick={async () => {
                    if (!hasChanges) {
                      setIsOpen(false);
                  ...
                  className="w-full h-11 rounded-lg text-sm font-semibold bg-primary hover:bg-primary/90 text-primary-foreground shadow-sm disabled:opacity-70 disabled:cursor-not-allowed"
                >
  ...
  return (
    <Card className="bg-card border border-border shadow-sm rounded-xl overflow-hidden flex flex-col flex-1">
      <CardHeader className="border-b border-border bg-muted/20 p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <CardDescription className="text-xs font-medium mt-0.5 text-muted-foreground">{description}</CardDescription>
          </div>
        </div>
      </CardHeader>
...
export const ConfigurationScreen: React.FC = () => {
  ...
  // Shared classes (minimal + theme-token based)
  const inputClasses = "mt-1.5 h-11 text-sm font-medium bg-background border border-border rounded-lg focus:border-ring transition-colors text-foreground shadow-sm";
  const labelClasses = "text-[11px] font-semibold text-muted-foreground uppercase tracking-wider";
  const boxClasses = "p-4 md:p-5 rounded-xl border border-border bg-muted/20 shadow-sm";

  return (
    <div className="p-4 md:p-8 max-w-[1400px] mx-auto min-h-full animate-in fade-in duration-300 pb-6 md:pb-8">
      
      {/* ── Header & Desktop Actions ── */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 md:mb-8 border-b border-border pb-4 md:pb-5">
        <div>
          <h1 className="text-xl md:text-2xl font-semibold tracking-tight text-foreground">System Configuration</h1>
          ...
          <div className="flex items-center gap-3">
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button variant="outline" className="h-10 px-4 gap-2 transition-colors active:scale-95 shadow-sm text-sm font-semibold rounded-lg">
              ...
              <AlertDialogContent className="bg-popover border border-border rounded-xl">
                <AlertDialogHeader>
                  <AlertDialogTitle className="text-base font-semibold text-foreground">Reset Configuration?</AlertDialogTitle>
                  <AlertDialogDescription className="text-sm font-medium text-muted-foreground">
                    This will reset all settings to their default values. This action cannot be undone.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter className="gap-2 mt-2">
                  <AlertDialogCancel className="h-10 border border-border rounded-lg font-semibold text-sm">Cancel</AlertDialogCancel>
                  <AlertDialogAction onClick={handleReset} className="h-10 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg font-semibold text-sm">Reset Settings</AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
            
            <Button onClick={handleSave} disabled={isSaving} className="h-10 px-5 gap-2 transition-transform active:scale-95 bg-primary hover:bg-primary/90 text-primary-foreground shadow-sm text-sm font-semibold rounded-lg disabled:opacity-70 disabled:cursor-not-allowed">
      ...
      <div className={`grid gap-4 md:gap-6 items-start ${isMobile ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'}`}>
        ...
        <div className="space-y-4 md:space-y-6 flex flex-col">
                    ...
                    {isAutoFlowActive ? (
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <Button
                          ...
                          disabled={autoFlowActionPending}
                          className="h-11 rounded-lg text-sm font-semibold tracking-wide transition-transform active:scale-95 bg-primary hover:bg-primary/90 text-primary-foreground shadow-sm"
                        >
                        ...
                        <Button
                          ...
                          disabled={autoFlowActionPending}
                          className="h-11 rounded-lg text-sm font-semibold tracking-wide transition-transform active:scale-95"
                        >
              ...
              */}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="pr-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <Label htmlFor="autoflow-mode" className="text-sm font-semibold text-foreground cursor-pointer">
                            Autoflow Mode
                          </Label>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      <Input
                        ...
                        className={`${inputClasses} pr-10 font-mono text-sm text-center`}
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-muted-foreground font-semibold uppercase mt-0.5">cm</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-border bg-card overflow-hidden shadow-sm">
                <div className="flex items-center justify-between p-4 border-b border-border bg-muted/20">
                  <div>
                    <h3 className="font-semibold text-sm text-foreground">Fixed Position Override</h3>
                    <p className="text-[11px] font-medium text-muted-foreground mt-0.5">Use precise known coordinates.</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Checkbox
                      ...
                      id="fixed-edit"
                    />
                    <Label htmlFor="fixed-edit" className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider cursor-pointer select-none">
                      Edit
                    </Label>
                  </div>
                </div>
                ...
                <div className="p-4 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                    ...
                    <Tabs value={fixedPositionView} onValueChange={(v) => setFixedPositionView(v as any)}>
                      <TabsList className="grid w-full grid-cols-2 bg-muted/40 p-1 rounded-lg border border-border h-10">
                        <TabsTrigger value="global" className="flex items-center justify-center text-[11px] sm:text-xs font-semibold rounded-md data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-foreground transition-all h-full text-muted-foreground">
                          GLOBAL
                        </TabsTrigger>
                        <TabsTrigger value="local" className="flex items-center justify-center text-[11px] sm:text-xs font-semibold rounded-md data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-foreground transition-all h-full text-muted-foreground">
                          LOCAL
                        </TabsTrigger>
                      </TabsList>
                    </Tabs>
                    ...
                    <div className="rounded-2xl border border-border bg-muted/20 p-2 shadow-sm">
                      <div className="grid grid-cols-2 gap-2">
                        <Button
                          type="button"
                          variant="ghost"
                          className="h-16 rounded-xl border border-border bg-background text-foreground hover:bg-muted/40 flex-col gap-1.5"
                        ...
                        <Button
                          type="button"
                          variant="ghost"
                          className="h-16 rounded-xl border border-destructive/30 bg-destructive/5 text-destructive hover:bg-destructive/10 flex-col gap-1.5"
                    ...
                    <Button
                      type="button"
                      className="w-full gap-2 bg-primary hover:bg-primary/90 text-primary-foreground transition-colors active:scale-95 font-semibold h-11 rounded-lg text-xs tracking-wide shadow-sm disabled:opacity-70 disabled:cursor-not-allowed"
          ...
          <SectionCard title="Streaming Configuration" description="Manage streaming links (NTRIP, LoRa)" icon={Globe} isMobile={isMobile} handleSave={handleSave} hasChanges={hasNtripChanges} actionLabel={isSaving ? 'Saving Details...' : 'Save Details'} actionPending={isSaving}>
            ...
            {/* NTRIP Block */}
            <div className="rounded-2xl border border-border bg-card/80 p-4 md:p-5 shadow-sm">
              <div className="flex items-center justify-between gap-3 mb-4">
                <div>
                  <div className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Streaming</div>
                  <div className="text-sm font-semibold text-foreground">NTRIP</div>
                </div>
              ...
              <Tabs defaultValue="sender" className="w-full flex flex-col">
                <div className="pb-3">
                  <TabsList className="grid w-full grid-cols-2 bg-muted/40 p-1 rounded-lg border border-border h-10">
                    <TabsTrigger value="sender" className="flex items-center justify-center text-[11px] sm:text-xs font-semibold rounded-md data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-foreground transition-all h-full text-muted-foreground">
                      <Upload className="size-3 mr-1.5 text-muted-foreground" /> SENDER
                    </TabsTrigger>
                    <TabsTrigger value="receiver" className="flex items-center justify-center text-[11px] sm:text-xs font-semibold rounded-md data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-foreground transition-all h-full text-muted-foreground">
                      <Download className="size-3 mr-1.5 text-muted-foreground" /> RECEIVER
                    </TabsTrigger>
                  </TabsList>
                </div>
                  ...
                  <div className="space-y-4 flex-1">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <button type="button" onClick={() => setShowPasswords({ ...showPasswords, ntripSender: !showPasswords.ntripSender })} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors p-2">
                ...
                <TabsContent value="receiver" className="m-0 space-y-4 animate-in fade-in slide-in-from-right-2 duration-300">
                  ...
                  <div className="space-y-4 flex-1">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <button type="button" onClick={() => setShowPasswords({ ...showPasswords, ntripReceiver: !showPasswords.ntripReceiver })} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors p-2">
                  ...
                  <Button
                    ...
                    className="w-full h-11 rounded-lg text-sm font-semibold tracking-wide transition-transform active:scale-95 shadow-sm mt-2"
            ...
            {/* LoRa Block */}
            <div className="mt-5 rounded-2xl border border-border bg-card/80 p-4 md:p-5 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Streaming</div>
                  <div className="text-sm font-semibold text-foreground">LoRa</div>
                  <div className="text-[11px] font-mono text-muted-foreground mt-1">
                    PORT: <span className="text-foreground">{streams.lora.port || 'NIL'}</span>
                  </div>
                </div>
                <Badge variant="outline" className={`text-[10px] font-semibold px-2 py-0.5 rounded-md ${
        ...
        <div className="space-y-4 md:space-y-6 flex flex-col">
          ...
          <SectionCard title="Telemetry Interfaces" description="Local data stream endpoints" icon={Activity} isMobile={isMobile} handleSave={handleSave} hasChanges={hasTelemetryChanges} actionLabel={isSaving ? 'Saving Changes...' : 'Save Changes'} actionPending={isSaving}>
              <Tabs defaultValue="serial" className="w-full flex flex-col">
                <div className="pb-3">
                  <TabsList className="grid w-full grid-cols-3 bg-muted/40 p-1 rounded-lg border border-border h-10">
                    <TabsTrigger value="serial" className="flex items-center justify-center text-[10px] sm:text-[11px] font-semibold rounded-md data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-foreground transition-all h-full text-muted-foreground">SERIAL</TabsTrigger>
                    <TabsTrigger value="tcp" className="flex items-center justify-center text-[10px] sm:text-[11px] font-semibold rounded-md data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-foreground transition-all h-full text-muted-foreground">TCP</TabsTrigger>
                    <TabsTrigger value="udp" className="flex items-center justify-center text-[10px] sm:text-[11px] font-semibold rounded-md data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-foreground transition-all h-full text-muted-foreground">UDP</TabsTrigger>
                  </TabsList>
                </div>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent className="bg-popover border border-border rounded-lg">
                        <SelectItem value="9600" className="font-mono text-sm focus:bg-accent">9600 bps</SelectItem>
                        <SelectItem value="19200" className="font-mono text-sm focus:bg-accent">19200 bps</SelectItem>
                        <SelectItem value="38400" className="font-mono text-sm focus:bg-accent">38400 bps</SelectItem>
                        <SelectItem value="57600" className="font-mono text-sm focus:bg-accent">57600 bps</SelectItem>
                        <SelectItem value="115200" className="font-mono text-sm focus:bg-accent">115200 bps</SelectItem>
                        <SelectItem value="230400" className="font-mono text-sm focus:bg-accent">230400 bps</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between pb-1 border-b border-border">
                      ...
                      <div className="flex gap-1.5">
                        <button onClick={() => handleMsmTypeChange('MSM4')} className={`px-2 py-1 text-[10px] font-bold uppercase rounded-md transition-all border ${activeMsgType === 'MSM4' ? 'bg-card border-border shadow-sm text-primary' : 'bg-transparent border-transparent text-muted-foreground hover:bg-muted/40 hover:text-foreground'}`}>MSM4</button>
                        <button onClick={() => handleMsmTypeChange('MSM7')} className={`px-2 py-1 text-[10px] font-bold uppercase rounded-md transition-all border ${activeMsgType === 'MSM7' ? 'bg-card border-border shadow-sm text-primary' : 'bg-transparent border-transparent text-muted-foreground hover:bg-muted/40 hover:text-foreground'}`}>MSM7</button>
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <button type="button" onClick={() => setShowPasswords({ ...showPasswords, wifi: !showPasswords.wifi })} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors p-2">
