...
const AppContent: React.FC = () => {
  ...
  if (!connection.isConnected && !survey.isActive && !isOfflinePreview) {
    return (
      <div className="min-h-dvh safe-x bg-background text-foreground transition-colors duration-300">
        <div key="connection-screen" className="animate-in fade-in zoom-in-[0.99] duration-200 ease-out">
          <ConnectionScreen />
        </div>
        <Toaster />
  ...
  return (
    <div className="h-dvh bg-background text-foreground flex overflow-hidden transition-colors duration-300">

      {/* ── Desktop Sidebar (Fixed Left) ── */}
      <aside className="hidden md:flex flex-col w-64 h-screen fixed desktop-sidebar-safe bg-sidebar text-sidebar-foreground border-r border-sidebar-border z-50">
        <div className="p-6">
      ...
      {/* ── Mobile Header (Fixed Top) ── */}
      <div className="md:hidden fixed top-0 mobile-fixed-safe bg-background/90 backdrop-blur-md border-b border-border z-50 pt-safe">
        <div className="flex items-center justify-between px-5 py-3">
          <div>
      ...
      {/* ── Mobile Interactive Bottom Navbar ── */}
      <nav className="md:hidden fixed bottom-0 mobile-fixed-safe z-50 bg-background/90 backdrop-blur-xl border-t border-border mobile-nav-safe">
        <div className="flex items-center justify-between px-3 py-2.5 gap-1">
          {navItems.map((item) => {
            const isActive = currentScreen === item.id;
            const Icon = item.icon;
            ...
            return (
              <button
                ...
                className={`relative flex items-center justify-center gap-2 px-3 py-3 rounded-2xl transition-all duration-500 ease-out active:scale-95 ${
                  isActive 
                    ? 'bg-primary text-primary-foreground shadow-sm flex-grow-[2]' 
                    : 'bg-transparent text-muted-foreground hover:text-foreground flex-grow-[1]'
                }`}
              >
                <Icon className={`size-5 transition-transform duration-300 ${isActive ? 'scale-110' : 'scale-100'}`} />
                {isActive && (
